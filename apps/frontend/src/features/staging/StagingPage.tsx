...
import { Archive, ArrowDown, Check, Copy, FileCode2, FileText, GitCommitHorizontal, History, Minus, Plus, Rows3 } from 'lucide-react'
...
export function StagingPage({ repository, onSnapshot, onNotice, onOperationChange, onOpenHistory, onOpenLineHistory }: StagingPageProps) {
  ...
  const unstagedFiles = useMemo(() => files.filter((file) => file.unstaged || (!file.staged && !file.unstaged)), [files])
  ...
  const updateStaging = async (paths: string[], mode: 'stage' | 'unstage') => {
    ...
    try {
      const updatedFiles = mode === 'stage'
        ? await stageRepositoryFiles(repository.path, paths)
  ...
  const stage = (paths: string[]) => updateStaging(paths, 'stage')
  ...
  const fileRow = (file: RepositoryFile, action: () => void, icon: 'add' | 'remove') => {
    const Icon = icon === 'add' ? Plus : Minus
    const FileIcon = file.path.includes('.') ? FileCode2 : FileText
    const status = getFileStatus(file.type)
    return <div className={`stage-file ${selectedPath === file.path ? 'active' : ''}`} key={`${icon}-${file.path}`} onContextMenu={(event) => { event.preventDefault(); setSelectedPath(file.path); setContextFile({ file, x: event.clientX, y: event.clientY }) }}>
