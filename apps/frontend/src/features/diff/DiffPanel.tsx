    ...
    <div className={`diff-body ${hideFileList ? 'code-only' : ''}`}>
      ...
      <div className={`code-diff ${view}`}>
        ...
        {!diffLoading && !diffError && view === 'unified' && rowsToRender.map((row, index) => { const side: DiffLineSide = row.kind === 'del' ? 'old' : 'new'; return <div data-diff-row-index={index} className={`code-row ${row.kind} ${lineMenu?.rowIndex === index && lineMenu.side === side ? 'line-context-active' : ''} ${activeChange?.fileIndex === activeFile && activeChange.rowIndex === index ? 'diff-navigation-active' : ''}`} key={index} onContextMenu={(event) => openLineMenu(event, row, index, side)} title="右键查看该行历史">
          <span className="line-no">{row.old ?? ''}</span><span className="line-no">{row.next ?? ''}</span><span className="change-sign">{row.kind === 'add' ? '+' : row.kind === 'del' ? '−' : ''}</span><HighlightedCode code={row.code} filePath={activeFileInfo?.path ?? ''}/>
        </div> })}
        {!diffLoading && !diffError && view === 'split' && <div className="split-diff-panes">
          <div className="split-diff-pane split-left" aria-label="旧版本代码">{splitRows.map((row, index) => <div data-diff-row-index={index} className={`split-code-side ${row.left?.kind ?? 'empty'} ${lineMenu?.rowIndex === index && lineMenu.side === 'old' ? 'line-context-active' : ''} ${activeChange?.fileIndex === activeFile && activeChange.rowIndex === index ? 'diff-navigation-active' : ''}`} key={index} onContextMenu={(event) => openLineMenu(event, row.left, index, 'old')} title={row.left ? '右键查看该行历史' : undefined}><span className="line-no">{row.left?.old ?? ''}</span><span className="change-sign">{row.left?.kind === 'del' ? '−' : ''}</span>{row.left ? <HighlightedCode code={row.left.code} filePath={activeFileInfo?.path ?? ''}/> : <code/>}</div>)}</div>
          <div className="split-diff-pane split-right" aria-label="新版本代码">{splitRows.map((row, index) => <div data-diff-row-index={index} className={`split-code-side ${row.right?.kind ?? 'empty'} ${lineMenu?.rowIndex === index && lineMenu.side === 'new' ? 'line-context-active' : ''} ${activeChange?.fileIndex === activeFile && activeChange.rowIndex === index ? 'diff-navigation-active' : ''}`} key={index} onContextMenu={(event) => openLineMenu(event, row.right, index, 'new')} title={row.right ? '右键查看该行历史' : undefined}><span className="line-no">{row.right?.next ?? ''}</span><span className="change-sign">{row.right?.kind === 'add' ? '+' : ''}</span>{row.right ? <HighlightedCode code={row.right.code} filePath={activeFileInfo?.path ?? ''}/> : <code/>}</div>)}</div>
        </div>}
      </div>
    </div>
