...
import { GitBranch, Mail, RefreshCw, Settings2, User, X } from 'lucide-react'
...
export function GitConfigDialog({ open, onClose, onNotice }: GitConfigDialogProps) {
  ...
  return <div className="modal-backdrop" onPointerDown={() => { if (!saving) onClose() }}>
    <section className="git-config-dialog" role="dialog" aria-modal="true" aria-labelledby="git-config-title" onPointerDown={(event) => event.stopPropagation()}>
      <div className="git-config-heading"><span className="git-config-icon"><Settings2 size={18}/></span><div><h2 id="git-config-title">本地 Git 配置</h2><p>管理当前系统的 Git 全局身份与常用默认行为。</p></div><button className="icon-button" onClick={onClose} disabled={saving} title="关闭"><X size={17}/></button></div>
      <form onSubmit={(event) => { event.preventDefault(); void save() }}>
        ...
        <div className="git-config-footer"><span className={error ? 'git-config-error' : ''}>{error ?? '配置保存到 ~/.gitconfig，并应用于本机的所有仓库。'}</span><div><button type="button" className="secondary-button" onClick={onClose} disabled={saving}>取消</button><button type="submit" className="primary-button" disabled={loading || saving}>{saving ? '正在保存…' : '保存配置'}</button></div></div>
      </form>
    </section>
  </div>
}
