...
export function CreateWorktreeDialog({ branches, busy, onClose, onSubmit }: {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !busy) onClose() }}>
    <section className="branch-dialog worktree-dialog" role="dialog" aria-modal="true" aria-label="创建 Worktree">
      <div className="branch-dialog-heading"><span className="branch-dialog-icon"><GitFork size={18}/></span><div><h2>创建 Worktree</h2><p>选择现有分支，或基于当前 HEAD 创建新分支。</p></div><button className="icon-button" onClick={onClose} disabled={busy} title="关闭"><X size={16}/></button></div>
      <form onSubmit={(event) => { event.preventDefault(); if (valid) void onSubmit(path.trim(), branch.trim(), mode === 'new') }}>
        ...
        <div className="branch-dialog-actions"><button type="button" className="secondary-button" onClick={onClose} disabled={busy}>取消</button><button type="submit" className="primary-button" disabled={!valid || busy}>{busy ? '正在创建…' : '创建 Worktree'}</button></div>
      </form>
    </section>
  </div>
}
