import { Box, Check, ChevronRight, FolderGit2, GitFork, Tag } from 'lucide-react'
import type { RepositorySnapshot } from '../../repository'

export type StructureView = 'worktrees' | 'submodules' | 'tags'

export function RepositoryStructurePanel({ repository, view, onOpenPath, onOpenTag }: { repository: RepositorySnapshot | null; view: StructureView; onOpenPath: (path: string, kind: 'worktree' | 'submodule') => void; onOpenTag: (tag: string) => void }) {
  const labels = {
  ...
  return <section className="structure-panel workspace-page">
    ...
    <div className="structure-list">
      ...
      {view === 'tags' && repository.tags.map((tag) => <button className="tag-row" key={tag} onClick={() => onOpenTag(tag)} title={`定位到标签 ${tag} 指向的提交`}><span className="structure-icon"><Tag size={15}/></span><span><strong>{tag}</strong></span><ChevronRight size={15}/></button>)}
